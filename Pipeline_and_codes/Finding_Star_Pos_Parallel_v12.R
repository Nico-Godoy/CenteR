...
Fun<-function(p,X,Y,Z,Xc,Yc,TF){
 if(p[1]>min(X) & p[1]<max(X) & p[2]>min(Y) & p[2]<max(Y) & sqrt(p[3]**2 + p[4]**2)>p[6] &
    p[3]<100*length(X)/2 &  p[4]<100*length(Y)/2 & p[7]>0 & p[8]>=0 &  abs(p[5])<=360 &
     p[6]<=2.2*100 & p[6]>=0.5*100 & p[8]/p[7]<0.99){# &
...
Tol<-seq(1e-5,0.1e-2,len=dim(P0)[1])
STM<-seq(1000,30000,len=dim(P0)[1])
TolG<-seq(1e-8,1e-3,len=dim(P0)[1])
...
#  while(SS<(dim(P0)[1]+1) ){
  ...
  SSi<-minV[1]
   ...
   if(SSi<=dim(P0)[1]){
    ...
    if(class(try(nlm(Fun,p=res$est,X=X,Y=Y,Z=Z[,,N],Xc=c(Xc,DXc),Yc=c(Yc,DYc),TF=TF,hessian=TRUE,iterlim = 8000,
         steptol = 1e-4,gradtol = 1e-4,stepmax=1000)))!='try-error'){
      res<-nlm(Fun,p=res$est,X=X,Y=Y,Z=Z[,,N],Xc=c(Xc,DXc),Yc=c(Yc,DYc),TF=TF,hessian=TRUE,iterlim = 8000,
         steptol = 1e-4,gradtol = 1e-4,stepmax=1000)
      SSi<-20000000
    }else if(class(try(nlm(Fun,p=res$est,X=X,Y=Y,Z=Z[,,N],Xc=c(Xc,DXc),Yc=c(Yc,DYc),TF=TF,hessian=TRUE,iterlim = 8000,
         steptol = 1e-1,gradtol = 1e-1,stepmax=1000)))!='try-error' & i!=1){
      res<-nlm(Fun,p=res$est,X=X,Y=Y,Z=Z[,,N],Xc=c(Xc,DXc),Yc=c(Yc,DYc),TF=TF,hessian=TRUE,iterlim = 8000,
         steptol = 1e-1,gradtol = 1e-1,stepmax=1000)
      SSi<-20000000
    }else{
      res<-list(est=c(9999+(1:Nind)*0),heSSiian=matrix(0,nr=Nind,nc=Nind),minimum=Inf)
      cat("Not found a non-singularity solution ...\n")
  ...
  if(class(try(solve(res$hessian)))=='matrix'){
     Err<-sqrt(abs(diag(Nind*res$minimum/1e4/(length(X)*length(Y) - Nind)*solve(res$hessian))))*1.96
  }else if(class(try(solve(res$hessian,tol=1e-8)))=='matrix'){
     Err<-sqrt(abs(diag(Nind*res$minimum/1e4/(length(X)*length(Y) - Nind)*solve(res$hessian,tol=1e-8))))*1.96
  }else if(class(try(solve(res$hessian,tol=1e-4)))=='matrix'){
     Err<-sqrt(abs(diag(Nind*res$minimum/1e4/(length(X)*length(Y) - Nind)*solve(res$hessian,tol=1e-4))))*1.96
  }else if(class(try(solve(res$hessian,tol=1e-1)))=='matrix'){
     Err<-sqrt(abs(diag(Nind*res$minimum/1e4/(length(X)*length(Y) - Nind)*solve(res$hessian,tol=1e-1))))*1.96
  }else{
     Err<-0*(1:Nind)
  }
