...
Fun2<-function(p,X,Z){
 ...
 CENT2<-function(A){
  ...
  E[,NonY]<- -99999
   ...
   P0x<-rbind(P0x,c(mean(x3),287,1.7))
   P0x<-rbind(P0x,c(mean(x3),290,1.7))
   P0x<-rbind(P0x,c(mean(x3),287,2.0))
   P0x<-rbind(P0x,c(mean(x3),287,1.5))
   P0x<-rbind(P0x,c(mean(x3),286,1.7))
   P0x<-rbind(P0x,c(mean(x3),288,1.7))
   P0x<-rbind(P0x,c(mean(x3),285,2))
   for(ip in -4:4){
     P0x<-rbind(P0x,c(mean(x3)+i*sd(x3),287,1.7))
   }

   P0y<-matrix(nr=0,nc=3)
   P0y<-rbind(P0y,c(mean(y3),287,1.7))
   P0y<-rbind(P0y,c(mean(y3),290,1.7))
   P0y<-rbind(P0y,c(mean(y3),287,2.0))
   P0y<-rbind(P0y,c(mean(y3),287,1.5))
   P0y<-rbind(P0y,c(mean(y3),286,1.7))
   P0y<-rbind(P0y,c(mean(y3),288,1.7))
   P0y<-rbind(P0y,c(mean(y3),285,2))
   for(ip in -4:4){
     P0y<-rbind(P0y,c(mean(y3)+i*sd(y3),287,1.7))
   }
   ...
   eiY<-1
   while(IsOk!=0){
    if(class(try(nlm(f,p=P0x[eiX,],x=x3,V1=V1.3,hessian=TRUE,iterlim=8000)))!='try-error'){
      Xap<-nlm(f,p=P0x[eiX,],x=x3,V1=V1.3,hessian=TRUE,iterlim=8000)
      IsOk<-0      
    }else{
      if(eiX!=length(P0x[,1])){
        eiX<-eiX+1
   ...
   IsOk<-1
   while(IsOk!=0){
    if(class(try(nlm(f,p=P0y[eiY,],x=y3,V1=V2.3,hessian=TRUE,iterlim=8000)))!='try-error'){
      Yap<-nlm(f,p=P0y[eiY,],x=y3,V1=V2.3,hessian=TRUE,iterlim=8000)
      IsOk<-0      
    }else{
      if(eiY!=length(P0y[,1])){
        eiY<-eiY+1
    ...
    if(Yap$min!=0){
     ...
     Xap0<-c(Xap$est[1],ErrX[1])  
     Yap0<-c(Yap$est[1],ErrY[1])
...
#   dev.off()
  ...
  if( Xc!=-9999  ){
   ...
   HotPix<-function(Z){
    ...
    Aux0<-which(  sqrt(  (XYaux[,1]-mean(Xaux))**2 + (XYaux[,2]-mean(Yaux))**2  )<=12  )
    Zaux00<-Z
    Zaux00[Aux0]<-NA
